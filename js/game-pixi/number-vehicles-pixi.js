 ...
 *
 * 전제:
 * - js/engine/pixi-runtime.js가 먼저 로드되어 있어야 한다.
 * - 기존 js/games/number-vehicles.js는 삭제하지 않는다.
 * - 이 파일은 별도 라인: window.SihyeonGames.numberVehiclesPixi 로 등록한다.
 *
 * 목표:
 * - 숫자 + 도형 + 색깔 + 자동차 꾸미기 통합형 Pixi 게임
 * - 모바일 세로 900×1600 / 태블릿 가로 1600×900 기준 좌표계
 * - 드래그 / 스냅 / 오답 복귀 / 꾸미기 / 출발 피날레 구현
 */
...
(function () {
  ...
  const VEHICLES = [
    {
      ...
      completeVoice: '소방차 출동!',
      finaleParticles: [0xff4d4d, 0xffd93d, 0x45c2ff, 0xffffff],
      parts: [
    ...
    },
    {
      ...
      emoji: '🚓',
      mainColor: 0xffffff,
      accentColor: 0x3a7bff,
      completeVoice: '경찰차 출동!',
      finaleParticles: [0x45c2ff, 0xffffff, 0xffd93d, 0x3a7bff],
      parts: [
    ...
    },
    {
      ...
      completeVoice: '구급차 출동!',
      finaleParticles: [0xff5b5b, 0xffffff, 0xff7ac8, 0x4ee6b8],
      parts: [
        { id: 'body', number: 1, label: '차체', shape: 'rectangle', color: 'white', rx: 0, ry: 46, w: 430, h: 138 },
      ...
      decor: [
        ...
        { id: 'sparkle', label: '반짝', icon: '✨' },
        { id: 'plus', label: '플러스', icon: '➕' }
      ]
    }
  ];
  ...
  function getRuntime() {
    return window.SihyeonPixiRuntime || null;
  }

  function getMode() {
    return state.runtime?.getMetrics?.().mode || 'portrait';
  }
  ...
  function getSceneConfig() {
    ...
    if (mode === 'landscape') {
      return {
        ...
        width: 1600,
        height: 900,
        carCenter: { x: 820, y: 455 },
        carScale: 1.08,
        trayStart: { x: 1375, y: 178 },
        trayGap: 92,
        trayColumns: 1,
        trayItemSize: 86,
    ...
    return {
      ...
      width: 900,
      height: 1600,
      carCenter: { x: 450, y: 690 },
      carScale: 0.86,
      trayStart: { x: 142, y: 1210 },
      trayGap: 128,
      trayColumns: 4,
      trayItemSize: 104,
  ...
  function gainExpOnce(amount) {
    if (!state.options || typeof state.options.gainExp !== 'function') return;
    state.options.gainExp(amount);
  }

  function fireConfetti() {
    if (state.options && typeof state.options.fireConfetti === 'function') {
  ...
  function drawSoftBackground() {
    ...
    g.beginFill(0x72cfff, 1);
    g.drawRect(0, 0, cfg.width, cfg.height);
    g.endFill();

    g.beginFill(0xbde9ff, 1);
    g.drawRect(0, 0, cfg.width, cfg.height * 0.5);
    g.endFill();

    g.beginFill(0x74c85a, 1);
    g.drawRect(0, cfg.height * 0.5, cfg.width, cfg.height * 0.5);
    g.endFill();
  ...
  function drawPanel(rect, options = {}) {
    ...
    const g = rt.createRoundRect({
      ...
      strokeAlpha: options.strokeAlpha ?? 0.88
    }, state.rootContainers.panel);

    return g;
  }

  function makeText(text, options, parent) {
    return state.runtime.createText(text, options, parent);
  }

  function getPartAbsPosition(part) {
    const cfg = getSceneConfig();
    return {
      x: cfg.carCenter.x + part.rx * cfg.carScale,
      y: cfg.carCenter.y + part.ry * cfg.carScale,
      w: part.w * cfg.carScale,
      h: part.h * cfg.carScale
    };
  }
  ...
  function drawShapeGraphic(part, sizeScale = 1, options = {}) {
    ...
    if (part.shape === 'wheel') {
      body.drawCircle(0, 0, Math.min(w, h) / 2);
      body.endFill();

      body.beginFill(0xffffff, 0.85);
      body.drawCircle(0, 0, Math.min(w, h) * 0.25);
      body.endFill();

      body.lineStyle(Math.max(3, 4 * sizeScale), 0x222222, 0.25);
      body.drawCircle(0, 0, Math.min(w, h) * 0.36);
      ...
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 18 * sizeScale);
      body.endFill();

      body.lineStyle(Math.max(5, 8 * sizeScale), 0xffffff, 0.86);
      body.moveTo(-w * 0.42, -h * 0.2);
      body.lineTo(w * 0.42, -h * 0.2);
      body.moveTo(-w * 0.42, h * 0.2);
      body.lineTo(w * 0.42, h * 0.2);

      for (let i = -3; i <= 3; i += 1) {
        const x = i * (w / 8);
        body.moveTo(x, -h * 0.32);
        body.lineTo(x, h * 0.32);
      }
    } else if (part.shape === 'cross') {
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 22 * sizeScale);
      body.endFill();
      ...
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 28 * sizeScale);
      body.endFill();
      ...
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 24 * sizeScale);
      body.endFill();

      body.beginFill(0xffffff, 0.38);
      body.drawRoundedRect(-w * 0.36, -h * 0.32, w * 0.28, h * 0.64, 12 * sizeScale);
      body.endFill();
      ...
      body.drawCircle(w * 0.28, 0, Math.max(5, 8 * sizeScale));
      body.endFill();
      ...
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 30 * sizeScale);
      body.endFill();

      body.lineStyle(Math.max(7, 10 * sizeScale), 0xffffff, 0.82);
      body.drawCircle(-w * 0.12, 0, Math.min(w, h) * 0.25);
      body.moveTo(w * 0.08, 0);
      body.lineTo(w * 0.38, -h * 0.18);
    } else if (part.shape === 'bag') {
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 18 * sizeScale);
      body.endFill();

      body.lineStyle(Math.max(4, 6 * sizeScale), 0xffffff, 0.88);
      body.drawRoundedRect(-w * 0.22, -h * 0.62, w * 0.44, h * 0.28, 12 * sizeScale);
    } else {
      body.drawRoundedRect(-w / 2, -h / 2, w, h, 24 * sizeScale);
      body.endFill();
    }
  ...
  function addNumberBadge(container, number, size = 46) {
    const rt = state.runtime;
    const badge = rt.createCircle({
      x: -size * 0.85,
      y: -size * 0.85,
      radius: size * 0.42,
      fill: 0xffffff,
      alpha: 1,
      strokeColor: 0xff9f1c,
      strokeWidth: 5
    }, container);

    badge.zIndex = 20;

    const text = makeText(number, {
      x: -size * 0.85,
      y: -size * 0.87,
  ...
  function createPartDisplay(part, options = {}) {
    ...
    const g = drawShapeGraphic(part, scale, {
      w: options.w || 112,
      h: options.h || 112,
    ...
    if (options.showNumber !== false) {
      addNumberBadge(c, part.number, options.badgeSize || 48);
    }

    if (options.x !== undefined && options.y !== undefined) {
      c.position.set(options.x, options.y);
    }

    c.hitArea = new rt.PIXI.Rectangle(
      -(options.hitW || 140) / 2,
      -(options.hitH || 140) / 2,
      options.hitW || 140,
      options.hitH || 140
  ...
  function createSlot(part) {
    ...
    if (part.shape === 'wheel') {
      ghost.drawCircle(0, 0, Math.min(pos.w, pos.h) / 2);
    } else if (part.shape === 'roof') {
      ghost.drawPolygon([
        -pos.w / 2, pos.h / 2,
        0, -pos.h / 2,
      ...
      ghost.drawRoundedRect(-pos.w / 2, -pos.h / 2, pos.w, pos.h, Math.max(16, 22 * cfg.carScale));
    }

    ghost.endFill();
    c.addChild(ghost);

    const label = makeText(part.number, {
      x: 0,
      y: 0,
      fontSize: Math.round(38 * cfg.carScale),
      fill: '#ffffff',
      stroke: '#17324a',
      strokeThickness: 5,
      dropShadow: false
    ...
    state.slotMap.set(part.id, {
      ...
      filled: state.placedPartIds.has(part.id)
    });
  ...
  function buildVehicleShadow() {
    ...
    const base = rt.createRoundRect({
      x: cfg.carCenter.x,
      y: cfg.carCenter.y + 84 * cfg.carScale,
      width: 520 * cfg.carScale,
      height: 210 * cfg.carScale,
      radius: 42 * cfg.carScale,
      fill: vehicle.mainColor,
      alpha: 0.12,
      strokeColor: 0xffffff,
      strokeWidth: 5,
      strokeAlpha: 0.3
    }, state.rootContainers.vehicle);

    base.zIndex = 1;

    const glass = rt.createRoundRect({
      x: cfg.carCenter.x - 66 * cfg.carScale,
      y: cfg.carCenter.y - 18 * cfg.carScale,
      width: 250 * cfg.carScale,
      height: 150 * cfg.carScale,
      radius: 42 * cfg.carScale,
      fill: 0xffffff,
      alpha: 0.08,
      strokeColor: 0xffffff,
  ...
  function renderSceneBase() {
    ...
    makeLayer('panel', rt.layers.world);
    makeLayer('vehicle', rt.layers.world);
    makeLayer('slots', rt.layers.objects);
    makeLayer('placed', rt.layers.objects);
    makeLayer('parts', rt.layers.objects);
    makeLayer('decor', rt.layers.objects);
    makeLayer('labels', rt.layers.ui);
    makeLayer('buttons', rt.layers.ui);
    ...
    makeText('숫자 탈것 만들기', {
      x: cfg.title.x,
      y: cfg.title.y,
      fontSize: cfg.mode === 'landscape' ? 42 : 48,
      fill: '#17324a',
      stroke: '#ffffff',
      strokeThickness: 7,
      wordWrapWidth: cfg.infoPanel.w - 36
    }, state.rootContainers.labels);

    const guideText = state.phase === 'decorate'
      ? `${vehicle.name}를 예쁘게 꾸며볼까?`
      : `${vehicle.name}를 만들어보자!`;

    makeText(guideText, {
      x: cfg.guide.x,
      y: cfg.guide.y,
      fontSize: cfg.mode === 'landscape' ? 24 : 30,
      fill: '#0d47a1',
      strokeThickness: 5,
      wordWrapWidth: cfg.infoPanel.w - 44
    }, state.rootContainers.labels);

    const progressText = state.phase === 'decorate'
      ? `꾸미기 ${state.placedDecorCount}/3`
      : `${Math.min(state.currentPartIndex + 1, vehicle.parts.length)}/${vehicle.parts.length} 단계`;

    makeText(progressText, {
      x: cfg.progress.x,
      y: cfg.progress.y,
      fontSize: cfg.mode === 'landscape' ? 34 : 36,
      fill: '#ff7a1a',
      strokeThickness: 6,
      wordWrapWidth: cfg.infoPanel.w - 44
    }, state.rootContainers.labels);
  ...
  function renderSlotsAndPlacedParts() {
    ...
    vehicle.parts.forEach((part) => {
      ...
      const pos = getPartAbsPosition(part);
      const placed = createPartDisplay(part, {
        parent: state.rootContainers.placed,
        x: pos.x,
        y: pos.y,
        w: pos.w,
        h: pos.h,
  ...
  function updateActiveSlotGlow() {
    ...
    const vehicle = state.currentVehicle;
    const target = vehicle.parts[state.currentPartIndex];
    if (!target) return;
    ...
    state.hintCleanup = state.runtime.hintGlow(slot.display, { color: 0xfff176 });
  }
  ...
  function getTrayPosition(index) {
    ...
    if (cfg.mode === 'landscape') {
      return {
        x: cfg.trayStart.x,
        y: cfg.trayStart.y + index * cfg.trayGap
      };
    }

    return {
      x: cfg.trayStart.x + col * cfg.trayGap,
      y: cfg.trayStart.y + row * cfg.trayGap
    };
  }

  function renderBuildTray() {
    const vehicle = state.currentVehicle;
    const available = vehicle.parts.filter((part) => !state.placedPartIds.has(part.id));

    available.forEach((part, index) => {
      const pos = getTrayPosition(index);
      const item = createPartDisplay(part, {
        parent: state.rootContainers.parts,
        x: pos.x,
        y: pos.y,
        w: getSceneConfig().trayItemSize,
        h: getSceneConfig().trayItemSize,
        scale: 0.72,
        badgeSize: 46,
        hitW: 132,
        hitH: 132
      ...
      state.runtime.makeDraggable(item, {
        ...
        onEnd: () => {
          state.isDragging = false;
          item.alpha = 1;
          handlePartDrop(item, part);
        },
        onCancel: () => {
          state.isDragging = false;
          returnPartToTray(item, part);
        }
      });
    });
  }
  ...
  async function placeCorrectPart(item, part, slot) {
    ...
    state.runtime.spawnParticles(slot.x, slot.y, {
      count: 18,
      colors: [COLOR_HEX[part.color] || 0xffffff, 0xffffff, 0xffd93d],
      duration: 620
    });
    ...
    if (state.currentPartIndex >= state.currentVehicle.parts.length) {
      state.locked = false;
      enterDecoratePhase();
      return;
    }
  ...
  function renderDecorTray() {
    const cfg = getSceneConfig();
    const vehicle = state.currentVehicle;
    const decorItems = vehicle.decor.slice(0, 3);
    ...
    decorItems.forEach((decor, index) => {
      ...
      state.runtime.createCircle({
        x: 0,
        y: 0,
        radius: cfg.mode === 'landscape' ? 52 : 58,
        fill: 0xffffff,
        alpha: 0.96,
        strokeColor: 0xff9f1c,
        strokeWidth: 6
      ...
      makeText(decor.icon, {
        x: 0,
        y: -2,
        fontSize: cfg.mode === 'landscape' ? 54 : 62,
        strokeThickness: 0,
        dropShadow: false
      }, c);
      ...
      state.runtime.makeDraggable(c, {
        ...
        onEnd: () => {
          state.isDragging = false;
          handleDecorDrop(c, decor);
        },
        onCancel: () => {
          state.isDragging = false;
          returnDecorToTray(c, decor);
        }
      });
    });
  ...
  async function handleDecorDrop(item, decor) {
    ...
    const insideCar =
      Math.abs(item.x - center.x) <= maxX &&
      Math.abs(item.y - center.y) <= maxY;

    if (!insideCar) {
      state.locked = true;
      await state.runtime.wrongShake(item);
      await returnDecorToTray(item, decor);
      state.locked = false;
      return;
    }
    ...
    state.runtime.spawnParticles(item.x, item.y, {
      count: 14,
      colors: [0xffd93d, 0xff7ac8, 0xffffff],
      duration: 520
    });
  ...
  function createStartButton() {
    ...
    rt.createRoundRect({
      x: 0,
      y: 0,
      width: cfg.actionButton.w,
      height: cfg.actionButton.h,
      radius: cfg.actionButton.h / 2,
      fill: 0x4caf50,
      alpha: 1,
      strokeColor: 0xffffff,
      strokeWidth: 7
    ...
    makeText('출동하기 🚀', {
      x: 0,
      y: -2,
      fontSize: cfg.mode === 'landscape' ? 30 : 38,
      fill: '#ffffff',
      stroke: '#1b5e20',
      strokeThickness: 5,
      dropShadow: true,
      wordWrapWidth: cfg.actionButton.w - 20
    }, button);

    rt.setInteractiveTap(button, () => {
      if (state.locked || state.phase !== 'decorate' || state.completing) return;
      completeVehicle();
    });
  }
  ...
  function renderInfoOnly() {
    ...
    makeText(`${vehicle.name}를 예쁘게 꾸며볼까?`, {
      x: cfg.guide.x,
      y: cfg.guide.y,
      fontSize: cfg.mode === 'landscape' ? 24 : 30,
      fill: '#0d47a1',
    ...
    makeText(`꾸미기 ${state.placedDecorCount}/3`, {
      x: cfg.progress.x,
      y: cfg.progress.y,
      fontSize: cfg.mode === 'landscape' ? 34 : 36,
      fill: '#ff7a1a',
  ...
  function enterDecoratePhase() {
    ...
    state.runtime.spawnParticles(getSceneConfig().carCenter.x, getSceneConfig().carCenter.y, {
      count: 45,
      colors: state.currentVehicle.finaleParticles,
      duration: 900
    });
  ...
  async function completeVehicle() {
    ...
    const cfg = getSceneConfig();
    const carObjects = [
      ...state.rootContainers.placed.children,
      ...state.rootContainers.decor.children
    ];
    ...
    carObjects.forEach((obj) => {
      ...
      obj.x = globalX - cfg.carCenter.x;
      obj.y = globalY - cfg.carCenter.y;
      group.addChild(obj);
    });

    group.position.set(cfg.carCenter.x, cfg.carCenter.y);

    state.runtime.spawnParticles(cfg.carCenter.x, cfg.carCenter.y, {
      count: 70,
      colors: state.currentVehicle.finaleParticles,
      duration: 1100
    });
    ...
    makeText('우와! 완성!', {
      x: cfg.width / 2,
      y: cfg.mode === 'landscape' ? 98 : 318,
      fontSize: cfg.mode === 'landscape' ? 58 : 66,
      fill: '#ff5b5b',
      stroke: '#ffffff',
      strokeThickness: 9,
      wordWrapWidth: 720
    ...
    state.runtime.setTimeout(() => {
      state.runtime.tween(group, {
        x: cfg.width + 520,
        y: cfg.carCenter.y - 30,
        rotation: 0.08
      }, 1400, 'easeInOutCubic');
    }, 500);
  ...
  function renderAll(options = {}) {
    const previousVehicle = state.currentVehicle;

    if (!options.preserve && !previousVehicle) {
      state.currentVehicle = VEHICLES[state.vehicleIndex];
    }
  ...
  async function render(container, options = {}) {
    ...
    if (!runtimeApi || typeof runtimeApi.createGame !== 'function') {
      container.innerHTML = `
        <div style="width:100%;height:100%;display:grid;place-items:center;text-align:center;font-family:Jua,sans-serif;font-size:24px;color:#17324a;background:#bde9ff;padding:24px;">
          Pixi Runtime을 불러오지 못했어요.<br>
          js/engine/pixi-runtime.js를 먼저 연결해 주세요.
        </div>
      `;
      return;
    }
    ...
    state.vehicleIndex = Math.floor(Math.random() * VEHICLES.length);
    state.currentVehicle = VEHICLES[state.vehicleIndex];

    resetRoundFlags();

    state.runtime = await runtimeApi.createGame(container, {
      backgroundAlpha: 0,
      onResize: () => {
        if (!state.runtime || state.isDragging || state.completing) return;
        renderAll({ preserve: true });
      }
    });
